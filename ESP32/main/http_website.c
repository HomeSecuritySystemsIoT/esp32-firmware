                          ...
                          int32_t event_id, void* event_data)
{
    ...
    if (event_base == ESP_HTTPS_SERVER_EVENT) {
        if (event_id == HTTPS_SERVER_EVENT_ERROR) {
            ...
            ESP_LOGE("event_handler", "Error event triggered: last_error = %s, last_tls_err = %d, tls_flag = %d", esp_err_to_name(last_error->last_error), last_error->esp_tls_error_code, last_error->esp_tls_flags);
        }
    }
}
 ...
 esp_err_t root_get_handler(httpd_req_t *req)
{
  ...
  const char * html="<!DOCTYPE html><html><head></head><body><h2>ESP32 WIFI</h2><form><label for=\"n\">Wifi SSID:</label><br><input type=\"text\"id=\"n\" name=\"n\"><br><br><label for=\"p\">WIFI Password:</label><br><input type=\"text\" id=\"p\" name=\"p\"><br><br><input type=\"submit\" value=\"Submit\"></form></body></html>";
	...
	printf("\nGOT SSID %u %.*s password %u %.*s\n",wifi_name.size,wifi_name.size,wifi_name.data,wifi_password.size,wifi_password.size,wifi_password.data);
...
static void print_peer_cert_info(const mbedtls_ssl_context *ssl)
{
    ...
    if (buf == NULL) {
        ESP_LOGE("print_peer_cert_info", "Out of memory - Callback execution failed!");
        return;
    }
    ...
    if (cert != NULL) {
        ...
        ESP_LOGW("print_peer_cert_info", "Could not obtain the peer certificate!");
    }
...
static void https_server_user_callback(esp_https_server_user_cb_arg_t *user_cb)
{
    ...
    switch(user_cb->user_cb_state) {
        case HTTPD_SSL_USER_CB_SESS_CREATE:
            ...
            if (esp_ret != ESP_OK) {
                ESP_LOGE("https_server_user_callback", "Error in obtaining the sockfd from tls context");
                break;
            }
            ...
            if (ssl_ctx == NULL) {
                ESP_LOGE("https_server_user_callback", "Error in obtaining ssl context");
                break;
            }
            // Logging the current ciphersuite
            ESP_LOGI("https_server_user_callback", "Current Ciphersuite: %s", mbedtls_ssl_get_ciphersuite(ssl_ctx));
#endif
            break;
 ...
 httpd_handle_t start_webserver(void)
{
	...
	//printf("%u %u %u %u %u\n",conf.max_uri_len ,conf.max_resp_headers,conf.max_open_sockets ,conf.stack_size,conf.max_header_len);
