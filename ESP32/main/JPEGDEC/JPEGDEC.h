...
#define __JPEGDEC__
	...
	defined(ESP_PLATFORM) || defined(_WIN64)
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#if defined(HAL_ESP32_HAL_H_) || defined(TEENSYDUINO) ||                       \
	defined(ARM_MATH_CM4) || defined(ARM_MATH_CM7) || defined(__x86_64__) ||   \
	defined(TEENSYDUINO)
...
enum { JPEG_MODE_BASELINE = 0, JPEG_MODE_PROGRESSIVE, JPEG_MODE_INVALID };
...
enum { JPEG_MEM_RAM = 0, JPEG_MEM_FLASH };
...
typedef int32_t(JPEG_READ_CALLBACK)(JPEGFILE *pFile, uint8_t *pBuf,
									int32_t iLen);
...
typedef struct jpeg_image_tag {
	...
	void *pUser;
	uint8_t
		*pDitherBuffer; // provided externally to do Floyd-Steinberg dithering
	...
	int16_t sUnalignedMCUs[8 + (DCTSIZE * MAX_MCU_COUNT)]; // 4:2:0 needs 6 DCT
														   // blocks per MCU
	void *pFramebuffer;
...
public:
	...
	int openFLASH(const uint8_t *pData, int iDataSize,
				  JPEG_DRAW_CALLBACK *pfnDraw);
	int open(const char *szFilename, JPEG_OPEN_CALLBACK *pfnOpen,
			 JPEG_CLOSE_CALLBACK *pfnClose, JPEG_READ_CALLBACK *pfnRead,
			 JPEG_SEEK_CALLBACK *pfnSeek, JPEG_DRAW_CALLBACK *pfnDraw);
#ifdef __LINUX__
...
int JPEG_getLastError(JPEGIMAGE *pJPEG);
void JPEG_setPixelType(JPEGIMAGE *pJPEG,
					   int iType); // defaults to little endian
...
#endif
#else
// Due to unaligned memory causing an exception, we have to do these macros the
// slow way
