...
int JPEGDEC::getPixelType() { return (int)_jpeg.ucPixelType; }
...
int JPEGDEC::openFLASH(const uint8_t *pData, int iDataSize,
					   JPEG_DRAW_CALLBACK *pfnDraw) {
	memset(&_jpeg, 0, sizeof(JPEGIMAGE));
...
int JPEGDEC::getLastError() { return _jpeg.iError; } /* getLastError() */

int JPEGDEC::getWidth() { return _jpeg.iWidth; } /* getWidth() */

int JPEGDEC::getHeight() { return _jpeg.iHeight; } /* getHeight() */

int JPEGDEC::hasThumb() { return (int)_jpeg.ucHasThumb; } /* hasThumb() */

int JPEGDEC::getThumbWidth() { return _jpeg.iThumbWidth; } /* getThumbWidth() */

int JPEGDEC::getThumbHeight() {
	return _jpeg.iThumbHeight;
} /* getThumbHeight() */

int JPEGDEC::getBpp() { return (int)_jpeg.ucBpp; } /* getBpp() */
...
//
int JPEGDEC::open(const char *szFilename, JPEG_OPEN_CALLBACK *pfnOpen,
...
//
int JPEGDEC::open(void *fHandle, int iDataSize, JPEG_CLOSE_CALLBACK *pfnClose,
...
static void FileClose(void *handle) { ((File *)handle)->close(); }
...
void JPEGDEC::close() {
	if (_jpeg.pfnClose) (*_jpeg.pfnClose)(_jpeg.JPEGFile.fHandle);
} /* close() */
