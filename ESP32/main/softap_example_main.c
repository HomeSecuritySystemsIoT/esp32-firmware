...
void app_main(void)
{
	...
	ret=connect(client_fd_udp,(struct sockaddr*)&dest_addr_udp,sizeof(dest_addr_udp));
	if(ret){
	...
	ret=connect(client_fd_tcp,(struct sockaddr*)&dest_addr_tcp,sizeof(dest_addr_tcp));
	if(ret){
